    ...
    const FetchData = async () => {
      try {
        if (bookingInfo.amount > 0) {
          await apis.Booking(bookingInfo).then((res) => {
            if (res.status === 200) {
              dispatch(
                actions.getInfoBooking({
                  ...bookingInfo,
    ...
    <div className="">
      <div className="flex flex-col containerr px-8 py-20 ">
        <TitleRoom
          ...
          sao={5}
          title={dataRoom?.name}
          introduce={dataRoom?.roomDTOs}
        />
      </div>
      <div className=" px-6">
        {/* sildes */}
        <div
          onClick={privousImages}
          className="flex justify-center gap-8 m-auto relative w-full rounded-3xl h-auto overflow-hidden "
        >
          {/* prev */}
          <button
            id="previous"
            type="button"
            className=" z-10 hover:bg-[#0e4f4f] hover:text-white 
          ...
          <button
            onClick={nextImages}
            type="button"
            className=" z-10 hover:bg-[#0e4f4f] hover:text-white border 
            ...
            <div className="flex flex-col gap-5">
              ...
              {dataRoom?.description !== null ? (
                <div className="flex flex-col gap-6">
                  <Utilitie
                    src={"../../icon/icon-utiliti.svg"}
                    util={dataRoom?.description}
                  />
                </div>
              ) : (
                <div className="block py-5 text-center roun bg-neutral-100">
                  <span className=" text-gray-500 text-sm font-semibold">
                    Hiện tiểu sử chưa được cập nhật
                  </span>
                </div>
              )}
            </div>
            ...
            <div className=" flex flex-col gap-5">
              ...
              <div className=" sm:px-56 py-8 rounded-3xl bg-[url('https://mixivivu.com/section-background.png')] flex flex-col gap-10 bg-[#f2f4f7]">
                ...
                {dataRoom.roomDTOs?.map((dtroom, key) => (
                  <InfoRoom key={key} dataInfoRoom={dtroom} max={dataRoom} />
                ))}
                <div className=" flex justify-between">
                  <div className="flex flex-col px-2">
                    ...
                    <span className=" pl-2 text-2xl font-bold text-[#0e4f4f]">
                      {format.FormatNumber(total * bookingInfo?.durationDays)}{" "}
                      VNđ
                    </span>
                  </div>
                  <button
                    className=" ease-in-out duration-200 rounded-md text-white px-6 py-2 bg-[#77dada] active:scale-95 hover:bg-[#5aa4a4]"
      ...
      {showError ? (
        <div className=" fixed top-24 right-6 w-80 text-center bg-neutral-50 shadow">
          ...
          <div className=" py-4 text-sm px-4 font-semibold text-red-400 ">
            {errors}
          </div>
        </div>
      ) : null}
