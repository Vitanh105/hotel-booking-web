...
function Pay() {
  ...
  const specificAddress =
    getinfoBK?.address?.addressLine +
    ", " +
    getinfoBK?.address?.district +
    ", " +
    getinfoBK?.address?.city +
    ", " +
    getinfoBK?.address?.country;
  ...
  const handleSubmit = (e) => {
    const FetchData = async () => {
      e.preventDefault();
      try {
        await apis.Payment(infoPay).then((res) => {
          if (res.status === 200) {
      ...
      <div className="py-5 flex sm:my-12 pb-64">
        ...
        <div className="flex flex-col shadow rounded-md border gap-3 flex-1 p-4 ">
          <div className="border p-4 rounded-lg flex flex-col gap-3 bg-gray-50">
            <div className=" font-bold text-xl">{getinfoBK?.nameHotel}</div>
            <div className=" text-sm">{specificAddress}</div>
          </div>
            ...
            <div className=" grid grid-cols-2">
              <div className="flex flex-col gap-2">
                ...
                <div className=" text-gray-600 text-sm">
                  {getinfoBK?.checkinDate}
                </div>
              </div>
              <div className="flex flex-col gap-2">
                <h4 className="font-semibold text-sm">Check out</h4>
                <div className="text-gray-600 text-sm">
                  {getinfoBK?.checkoutDate}
                </div>
              </div>
            </div>
            <div className="flex flex-col gap-2 text-sm">
              <h4 className="font-semibold ">Thời gian cư trú</h4>
              <div className="text-gray-600">{getinfoBK?.durationDays} Đêm</div>
            </div>
        ...
        <form
          action="/pay"
          className="flex flex-col justify-between flex-[2_1_0%] px-5 gap-10"
          onSubmit={handleSubmit}
        >
          <div className=" flex gap-5 items-center">
            <h3 className="font-bold text-lg">Tổng tiền: </h3>
            <div className=" text-gray-800 font-semibold">
              {format.FormatNumber(getinfoBK?.amount)} VNđ
            </div>
          </div>
