...
function AddManagerHotel() {
  ...
  function handleChangeRoomSing(e) {
    ...
    switch (name) {
      case "service":
        ...
        return setRoomSing((prev) => ({ ...prev, serviceDTOs: resule }));

      case "roomCount":
        if (roomSing?.roomCount >= 0) {
          return setRoomSing((prev) => ({
            ...prev,
            roomCount: parseInt(value, 10),
          }));
        }
      ...
      case "pricePerNight":
        if (roomSing.pricePerNight >= 0) {
          setRoomSing((prev) => ({
            ...prev,
            pricePerNight: parseInt(value, 10),
          }));
        }
      default:
        return setRoomSing({ ...roomSing, [name]: value });
    }
  }
  ...
  function handleChangeRoomDou(e) {
    ...
    switch (name) {
      case "service":
        ...
        return setRoomDou((prev) => ({ ...prev, serviceDTOs: resule }));

      case "roomCount":
        if (roomDou?.roomCount > 0) {
          return setRoomDou((prev) => ({
            ...prev,
            roomCount: parseInt(value, 10),
          }));
        }
      ...
      default:
        return setRoomDou({ ...roomDou, [name]: value });
    }
  }
  ...
  function handleChangeRoomFami(e) {
    ...
    switch (name) {
      case "service":
        ...
        return setRoomFami((prev) => ({ ...prev, serviceDTOs: resule }));

      case "roomCount":
        if (roomFami?.roomCount > 0) {
          return setRoomFami((prev) => ({
            ...prev,
            roomCount: parseInt(value, 10),
          }));
        }
      ...
      default:
        return setRoomFami({ ...roomFami, [name]: value });
    }
  }
  ...
  function handleSubmit(e) {
    const FetchEdit = async () => {
      try {
        e.preventDefault();
        await apis.AddManagerHotel(valueAdd).then((res) => {
          console.log(res);
          if (res.status === 201) {
            return (
